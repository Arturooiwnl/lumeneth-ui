...
export type CodeBlockCommandProps = {
    /** Key-value pair mapping each package manager to its specific terminal command. */
    commands?: Record<PackageManager, string>
    /** Additional Tailwind layout adjustments for the container box. */
    className?: string
}

const DEFAULT_COMMANDS: Record<PackageManager, string> = {
    pnpm: "pnpm dlx shadcn add button",
    yarn: "yarn dlx shadcn add button",
    npm: "npx shadcn add button",
    bun: "bun x shadcn add button",
}
...
export function CodeBlockCommand({
    ...
    const handleCopy = async () => {
        try {
            ...
            console.error("Failed to copy command context:", err)
        }
    }
    ...
    return (
        <div
            className={cn(
                "w-full max-w-xl rounded-xl border border-border/50 shadow-2xl overflow-hidden font-mono text-sm bg-background",
                className
            )}
        >
            <div className="flex items-center justify-between px-4 border-b border-border/50 bg-card/50 h-12 select-none gap-2">
                <div className="flex items-center gap-3 h-full flex-1 min-w-0">
                    <motion.div
                        key={activeTab}
                        initial={{ opacity: 0, filter: "blur(4px)" }}
                        animate={{ opacity: 1, filter: "blur(0px)" }}
                        transition={{ duration: 0.18, ease: "easeOut" }} className="size-4 shrink-0 text-muted-foreground/80">
                        {getIconForPackageManager(activeTab)}
                    </motion.div>
                    <nav className="flex items-center gap-1 h-full overflow-x-auto relative scrollbar-none [&::-webkit-scrollbar]:hidden">
                        {tabs.map((tab) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={cn(
                                        "relative px-2.5 h-full flex items-center text-xs transition-colors duration-200 outline-none focus-visible:text-foreground shrink-0 whitespace-nowrap",
                                        isActive
                ...
                <button
                    onClick={handleCopy}
                    className="p-2 rounded-md hover:bg-accent/50 text-muted-foreground/50 hover:text-foreground transition-all active:scale-95 shrink-0 ml-2"
                ...
                >
                    {isCopied ? (
                        ...
                        <Copy className="size-3.5 group-hover:scale-105 transition-transform" />
                    )}
                </button>
            </div>

            <div className="w-full p-5 overflow-x-auto bg-card/50 antialiased scrollbar-thin scrollbar-thumb-zinc-800/50">
                <div className="flex items-center gap-2.5 min-w-max">
                    <span className="text-indigo-300/80 shrink-0 select-none">$</span>
                    <motion.span
                        key={activeTab}
                        initial={{ opacity: 0, x: -4, filter: "blur(4px)" }}
                        animate={{ opacity: 1, x: 0, filter: "blur(0px)" }}
                        transition={{ duration: 0.18, ease: "easeOut" }}
                        className="text-foreground/80 tracking-tight whitespace-nowrap inline-block"
                    >
                        {commands[activeTab]}
                    </motion.span>
                </div>
